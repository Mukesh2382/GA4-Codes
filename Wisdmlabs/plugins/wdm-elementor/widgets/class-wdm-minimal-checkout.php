...
/**
 * WdmMinimalCheckout to create a new custom widget
 */
class WdmMinimalCheckout extends \Elementor\Widget_Base {
	...
	public function __construct($data = [], $args = null) {
		...
		if($this->is_widget_active('wdmminimalcheckout')){
			wp_deregister_script( 'edd-ajax' );
			wp_register_script( 'edd-ajax', WDMELE_PLUGIN_URL.'assets/js/edd-ajax.js', array( 'jquery', 'wdm-script-handle-minimal-checkout' ), EDD_VERSION, $in_footer );
			// wp_register_script( 'edd-ajax', WDMELE_PLUGIN_URL.'assets/js/edd-ajax.js', array( 'jquery', 'wdm-script-handle-minimal-checkout' ), NULL, true );
			...
			wp_register_script( 'wdm-script-handle-minimal-checkout', WDMELE_PLUGIN_URL.'assets/js/minimal-checkout.js', [ 'elementor-frontend' ], '1.0.0', true );
			wp_localize_script( 'wdm-script-handle-minimal-checkout', 'wdmCheckoutAjax', array( 'decimal_separator' => edd_get_option( 'decimal_separator', '.' ),'currency_sign'=> edd_currency_filter(''),'enter_discount'=>'Enter discount', 'asset_path' => WDMELE_PLUGIN_URL.'assets', 'ajaxurl' => admin_url( 'admin-ajax.php' )));
			// wp_localize_script( 'wdm-script-handle-minimal-checkout', 'wdmAjax', array( 'asset_path' => WDMELE_PLUGIN_URL.'assets', 'ajaxurl' => admin_url( 'admin-ajax.php' )));
			wp_register_style( 'wdm-style-handle-minimal-checkout', WDMELE_PLUGIN_URL.'assets/css/minimal-checkout.css');
		}  
	...
	public function get_name() {
		return 'wdmminimalcheckout';
	}

	public function get_title() {
		return __( 'Wisdm Minimal Checkout', 'wdm-elementor-addon-extension' );
	}
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			'wdm_minimal_checkout_content_section',
			[
        ...
        $this->add_control(
			'wdm_minimal_checkout_description',
			[
		...
		$this->add_control(
			'wdm_minimal_checkout_privacy_policy',
			[
		...
		$this->add_control(
			'wdm_minimal_checkout_price_details_heading',
			[
	...
	public function wdm_minimal_checkout_reviews(){
		...
		foreach( $reviews as $review ){
			...
			// $details = convert_smilies(strip_tags(htmlspecialchars_decode($translate["text"])));
			?>
	...
	/**
     * generate_cart_section To generate cart table on minimal checkout page
     *
     * @return void
     */
    public function generate_cart_section(){
        ...
        <table id="wdm_edd_checkout_cart">
            ...
            <tfoot>
                ...
                if($discounts){
                    $style = '';
					$discount_amount = edd_get_cart_subtotal() - EDD()->cart->get_total();
                }
                ?>
				<tr class="wdm_edd_cart_footer_row sub-discount-row" style="<?php echo $style?>">
					<th class="edd_cart_total edd_cart_discount_text"><?php echo esc_html( __( 'Discount', 'wdm-elementor' ) );?></th>
					<th class="edd_cart_amount_row edd_cart_discount_row"><span class="discount-minus-sign">-</span><span class="edd_cart_amount edd_cart_discount"><?php echo esc_html( edd_currency_filter( edd_format_amount( number_format( $discount_amount ) ) ) ); ?></span></th>
				</tr>
                <tr class="wdm_edd_cart_footer_row sub-subtotal-row" style="<?php echo $style?>">
                    <th class="edd_cart_total">Net Payable</th>
                    <!-- <th></th> -->
                    <th class="edd_cart_amount_row"><span class="edd_cart_amount" data-subtotal="<?php echo edd_get_cart_subtotal()?>" data-total="<?php echo edd_get_cart_total()?>"><span class="discounted-price"><?php echo edd_cart_total( false )?></span></span></th>
                    <!-- <th></th> -->
                </tr>
                <?php
                unset($discounts);
	...
	/**
     * generate_discount_section To generate discount fields and label section
     *
     * @return void
     */
        ...
        <div id="wdm_discount_coupon">
            ...
            <div id="wdm_edd_discount_wrap" style="">
                <span href="javascript:void(0);" target="_self" class="wdm_edd_discount_applied wdm_edd_discount_remove"
                    ...
                    <span id="wdm_coupon_amount"> <?php echo apply_filters('wdm_checkout_page_discount_applied_rate', $rate, $discount); ?> </span>
	...
	public function generate_minimal_apply_coupon_check($discount=''){
		?>
		<label class="apply_check_label checkbox-container" for="apply_discount_check"> Apply Coupon<input type="checkbox" id="apply_discount_check" class="apply_discount_check" name="apply_discount_check" value="1" <?php echo $discount?'checked':''?>><span class="checkmark"></span></label>
		<?php
	}

	public function generate_minimal_discount_field_input($discount=''){
		$email = $current_user?$current_user->user_email:'';
		?>
		<div class="wdm-block-discount-field">
			<div class="coupon-fields">
	...
	public function generate_login_links_form($provider='google'){
		echo do_shortcode( '[theme-my-login action="login" login_template="wdm-minimal-login-form.php"]' );
	}
	...
	public function get_script_depends(){
		return ['wdm-script-handle-minimal-checkout'];
	}

	public function get_style_depends(){
		return ['wdm-style-handle-minimal-checkout'];
	}
