...
class Wisdm_Edd_Checkout {
    ...
    private function __construct(){
        ...
        add_action( 'edd_built_order', array( $this,'wdm_edd_store_custom_fields'));
        add_action( 'edd_payment_billing_details', array( $this, 'wdm_edd_show_custom_fields' ) );
	...
	public function edd_user_info_fields() {
        ...
        <fieldset id="edd_cc_address" class="edwiser-minimal-checkout-cc-address-wrapper cc-address">
            ...
            <?php 
            // Show the GST Number and GST Business Address fields to Indian users only.
            $stored_currency = edd_currency_get_stored_currency();
            $gateways        =  edd_get_enabled_payment_gateways();
            // Check if currency is INR and payment gateway is razorpay.
            //if( 'INR' == $stored_currency && isset( $gateways['razorpay'] ) ){

            ?>
                
            <p id="edd-label-p">
                <label class="edd-label">
                    <?php _e( 'Pay With:', 'easy-digital-downloads' ); ?>
                </label>                
            </p>
            <div class="radio-group-div">
                <span class="radio-group">
                    <input type="radio" id="pp_pay" name="fav_language" value="paypal" checked>
                    <label for="pp_pay" class="wdm_payment">Paypal</label>
                </span>
                <span class="radio-group">
                    <input type="radio" id="rzr_pay" name="fav_language" value="razor_pay">
                    <label for="rzr_pay" class="wdm_payment">Razor Pay (For India based customers)</label><br>
                </span>
            </div>
                <style>
                    .radio-group-div{
                    ...
                    .wdm-block-checkout-container #edd_checkout_wrap fieldset #edd-label-p{
                        margin-bottom: 10px;
                    }

                    #edd_cc_address .radio-group label {
                        padding: 8px 15px;
                    }
                    #edd_cc_address .radio-group input[type="radio"]:checked + label {
                        background-color: #f75d25;
                        color: #fff;
                    }
                    .wdm-my-account-wrap .col{
                        width : auto;
                    }
                    #edd_checkout_login_register span{
                        font-size: 50%;
                    }
                    span.radio-group:last-child {
                        padding-left: 40px;
                    }
                    #edd_cc_address .radio-group-div {
                        display: flex;
                        justify-content: flex-start;
                    }

                    .ed-add-setup-package-heading{
                        line-height: 1.85714285714286;
                    }

                    .radio-group-div .radio-group label {
                        display: inline-block;
                        cursor: pointer;
                        padding: 8px 15px;
            ...
            <p id="wdm-custom-edd-gst-number" style="display: none;" >
                <label for="wdm_edd_gst_number" class="edd-label">
                    <?php _e( 'GST Number', 'easy-digital-downloads' ); ?>
                    <span class="edd-required-indicator">*</span>
                </label>
                <textarea id="wdm_edd_gst_number" name="wdm_edd_gst_number" class="wdm_edd_gst_number edd-input required" rows="1"></textarea>
            </p>
            <p id="wdm-custom-edd-gst-business-address" style="display: none;">
                <label for="wdm_edd_gst_business_address" class="edd-label">
                    <?php _e( 'GST Business Address', 'easy-digital-downloads' ); ?>
                    <span class="edd-required-indicator">*</span>
                </label>
                <textarea id="wdm_edd_gst_business_address" name="wdm_edd_gst_business_address" class="wdm_edd_gst_business_address edd-input required" rows="1"></textarea>
            </p>
        </fieldset>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#pp_pay').change(function() {
                    if ($(this).is(':checked')) {
                        // Hide the first two <p> tags
                        $('p[id^="wdm-custom-edd-gst"]').hide();
                    } else {
                        // Show the first two <p> tags
                        $('p[id^="wdm-custom-edd-gst"]').show();
                    }
                });
            });
        </script>
    ...
    public function generate_cart_section(){
        ...
        $download_ids_hide_license_options = array(
            '95719',
            '7249',
            '223403',
        );
            ...
            <tbody>
                <?php 
                    foreach ( $cart_contents as $item_key=>$item ) {
                        ...
                        $license_options_html = in_array( $item['id'], $download_ids_hide_license_options ) ? '' : '<div class="wdm_edd_cart_item_licenses download_' . esc_attr( $item['id'] ) . '" data-nonce="'.$this->cart_nonce.'" data-cart-key="'.$item_key.'" >'.$this->get_license_options_html($license_options,$item_key).'</div>';
    ...
    /**
     * Save the GST Number and GST Business Address data in the database.
     * The data is saved when the customer purchases the product from the checkout page.
     */
    public function wdm_edd_store_custom_fields( $order_id ) {
        if ( 0 !== did_action('edd_pre_process_purchase') ) {
            $wdm_edd_gst_number = isset( $_POST['wdm_edd_gst_number'] ) ? sanitize_textarea_field( $_POST['wdm_edd_gst_number'] ) : '';
            edd_add_order_meta( $order_id, 'wdm_edd_gst_number', $wdm_edd_gst_number );

            $wdm_edd_gst_business_address = isset( $_POST['wdm_edd_gst_business_address'] ) ? sanitize_textarea_field( $_POST['wdm_edd_gst_business_address'] ) : '';
            edd_add_order_meta( $order_id, 'wdm_edd_gst_business_address', $wdm_edd_gst_business_address );
        }
    }

    /**
     * Show the saved GST Number and GST Business Address on the Edd Order
     * backend page.
     * The data is shown on Order Details section under Address sub section.
     */
    public function wdm_edd_show_custom_fields( $order_id ) {
        $wdm_edd_gst_number           = edd_get_order_meta( $order_id, 'wdm_edd_gst_number', true );
        $wdm_edd_gst_business_address = edd_get_order_meta( $order_id, 'wdm_edd_gst_business_address', true );

        ?>
        <div id="wdm-edd-details">
            <div class="wdm-edd-gst-details-order">
                <h3>GST Details</h3>
                <div class="edd-form-group">
                    <label class="edd-form-group__label">GST Number:</label>
                    <div class="edd-form-group__control">
                        <?php echo esc_html( $wdm_edd_gst_number ); ?>
                    </div>
                </div>
                <div class="edd-form-group">
                    <label class="edd-form-group__label">GST Business Address:</label>
                    <div class="edd-form-group__control">
                        <?php echo esc_html( $wdm_edd_gst_business_address ); ?>
                    </div>
                </div>
            </div>
        </div>
